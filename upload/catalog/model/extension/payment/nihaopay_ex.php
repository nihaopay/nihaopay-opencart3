<?php
class ModelExtensionPaymentNihaoPayEx extends Model {
	public function getMethod($address, $total) {
		...
		if ($this->config->get('payment_nihaopay_ex_total') > 0 && $this->config->get('payment_nihaopay_ex_total') > $total) {
			$status = false;
		} elseif (!$this->config->get('payment_nihaopay_ex_geo_zone_id')) {
			$status = true;
		...
		if ($status) {
			$method_data = array(
				...
				'terms'      => '',
				'sort_order' => $this->config->get('payment_nihaopay_ex_sort_order')
			);
		}
